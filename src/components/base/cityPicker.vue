    ...
    watch: {
      items () {
        ...
        for (let i = 0; i < this.items.length; i++) {
          itemArray.push(this.items[i].proName);
          itemMap[this.items[i].proName] = {id: this.items[i].id};

          if (this.defaultProvinceId !== null && this.defaultProvinceId !== '' && this.defaultProvinceId === this.items[i].id) {
            defaultProvinceName = this.items[i].proName;
          }

          if (this.items[i].cityList) {
            let secondMap = {};
            let secondArray = [];
            let items = this.items[i].cityList;
            ...
            itemMap[this.items[i].proName].map = secondMap;
            mapArray[this.items[i].proName] = secondArray;
          }
        }
        ...
        this.itemSlots[1].values = mapArray[this.items[0].proName];


        if (defaultProvinceName !== null && defaultCityName !== null) {
          this.itemChange(defaultProvinceName, 0);
          this.itemChange(defaultCityName, 1);
          this.itemNameStr = this.proName + '-' + this.name;
          this.itemName = [this.proName, this.name];
        } else {
          this.itemChange(this.items[0].proName, 0);
          this.itemChange(this.items[0].cityList[0].name, 1);
          if (this.showDefault) {
            this.itemNameStr = this.proName + '-' + this.name;
          }
          this.itemName = [this.proName, this.name];
    ...
    methods: {
      ...
      cancelChoose () {
        ...
        if (this.itemNameStr !== '请选择') {
          this.proName = this.itemNameStr.split('-')[0];
          this.name = this.itemNameStr.split('-')[1];
          this.itemSlots[1].values = this.mapArray[this.proName];
          this.itemName = [this.proName, this.name];
        }
      }
    }
